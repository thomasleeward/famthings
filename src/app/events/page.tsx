      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
                  ...
                  <div className="space-y-3">
                    {monthEvents.map((event) => (
                      <Card className="flex items-center gap-4 p-4" key={event.id}>
                        <div className="grid size-20 shrink-0 place-items-center rounded-lg bg-cream text-center">
                          <div>
                            <p className="text-xs font-extrabold uppercase text-muted">{event.date.slice(0, 3)}</p>
                            <p className="text-3xl font-black">{event.date.match(/\d+/)?.[0]}</p>
                          </div>
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-xl font-black">{event.title}</p>
        ...
        <Card className="h-fit p-5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-black">Event builder</h2>
          </div>
          <div className="mt-4 space-y-3">
            <input className="h-11 w-full rounded-lg border border-line bg-cream px-3 font-medium" placeholder="Event title" />
            ...
            <div className="rounded-lg border border-line bg-cream p-3">
              <p className="text-xs font-extrabold uppercase tracking-[0.14em] text-muted">Linked to-dos</p>
              <p className="mt-2 text-sm text-muted">
                To-dos created here insert into <span className="font-bold text-ink">todos.event_id</span>, so they also show on the To-dos page.
              </p>
              <div className="mt-3 space-y-2">
                {todos.filter((todo) => todo.eventId).map((todo) => (
                  <div className="rounded-lg bg-white px-3 py-2 text-sm font-bold" key={todo.id}>{todo.title}</div>
                ))}
              </div>
            </div>
